 ...
 */
public class ReadXMLUsingSAX implements ICommand {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            List<BankDeposit> deposits = ReaderFactory.getInstance()
                    .getSAXReader().readXML(DataPath.XML_FILE);

            request.setAttribute("type", ParserType.SAX);
            request.setAttribute("deposits", deposits);
            return PageName.DEPOSITS_PAGE;
        } catch (XMLReaderSAXException ex) {
            log.error(ex);
        }
